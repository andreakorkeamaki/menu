...
export default async function TeamPage({ searchParams }: { searchParams: Promise<{ invited?: string; error?: string }> }) {
  ...
  return (
    <main className="workspace">
      ...
      {params.invited && <p className="form-success" role="status">{params.invited === "existing" ? "Utente Auth già esistente: accesso collegato senza un nuovo invito." : "Invito inviato."}</p>}{params.error && <p className="form-error" role="alert">Invito non riuscito: verifica email e autorizzazioni.</p>}

...
export default async function MediaReviewPage({ searchParams }: { searchParams: Promise<{ reviewed?: string; error?: string; page?: string }> }) {
  ...
  const [mediaResult, logoResult, coverResult, dishResult] = await Promise.all([
    supabase!.from("media_assets")
      .select("id,organization_id,location_id,menu_item_id,object_path,media_kind,mime_type,alt_text,created_at,organization:organizations(name),location:locations(name),item:menu_items(name_it)", { count: "exact" })
      .eq("bucket_id", "intake")
  ...
  return (
    <main className="workspace wide-workspace">
      <header className="workspace-heading"><div><p className="eyebrow">Controllo qualità</p><h1>Immagini prima della pubblicazione</h1><p>Verifica nitidezza, pertinenza e diritti d’uso. L’approvazione crea una copia WebP ottimizzata; l’originale privato non viene mai servito agli ospiti.</p></div><span className="review-safety-badge">Intake privato</span></header>

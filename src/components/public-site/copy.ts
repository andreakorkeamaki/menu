import type { Locale } from "@/types/domain";

export const PUBLIC_COPY: Record<
  Locale,
  {
    navMenu: string;
    navInfo: string;
    reserve: string;
    menuEyebrow: string;
    searchLabel: string;
    searchPlaceholder: string;
    noResults: string;
    unavailable: string;
    vegetarian: string;
    vegan: string;
    glutenFree: string;
    allergens: string;
    ingredients: string;
    information: string;
    hours: string;
    contacts: string;
    directions: string;
    whatsapp: string;
    instagram: string;
    language: string;
    publishedMenu: string;
  }
> = {
  it: {
    navMenu: "Menu",
    navInfo: "Informazioni",
    reserve: "Prenota",
    menuEyebrow: "Il nostro menu",
    searchLabel: "Cerca nel menu",
    searchPlaceholder: "Cerca un piatto o un ingrediente…",
    noResults: "Nessun piatto corrisponde alla ricerca.",
    unavailable: "Non disponibile",
    vegetarian: "Vegetariano",
    vegan: "Vegano",
    glutenFree: "Senza glutine",
    allergens: "Allergeni",
    ingredients: "Ingredienti",
    information: "Vieni a trovarci",
    hours: "Orari",
    contacts: "Contatti",
    directions: "Apri la mappa",
    whatsapp: "Scrivici su WhatsApp",
    instagram: "Instagram",
    language: "Lingua",
    publishedMenu: "Menu digitale",
  },
  en: {
    navMenu: "Menu",
    navInfo: "Information",
    reserve: "Book a table",
    menuEyebrow: "Our menu",
    searchLabel: "Search the menu",
    searchPlaceholder: "Search for a dish or ingredient…",
    noResults: "No dishes match your search.",
    unavailable: "Unavailable",
    vegetarian: "Vegetarian",
    vegan: "Vegan",
    glutenFree: "Gluten-free",
    allergens: "Allergens",
    ingredients: "Ingredients",
    information: "Visit us",
    hours: "Opening hours",
    contacts: "Contact",
    directions: "Open map",
    whatsapp: "Message us on WhatsApp",
    instagram: "Instagram",
    language: "Language",
    publishedMenu: "Digital menu",
  },
  fr: {
    navMenu: "Menu",
    navInfo: "Informations",
    reserve: "Réserver",
    menuEyebrow: "Notre menu",
    searchLabel: "Rechercher dans le menu",
    searchPlaceholder: "Rechercher un plat ou un ingrédient…",
    noResults: "Aucun plat ne correspond à votre recherche.",
    unavailable: "Indisponible",
    vegetarian: "Végétarien",
    vegan: "Végan",
    glutenFree: "Sans gluten",
    allergens: "Allergènes",
    ingredients: "Ingrédients",
    information: "Venez nous voir",
    hours: "Horaires",
    contacts: "Contact",
    directions: "Ouvrir la carte",
    whatsapp: "Écrivez-nous sur WhatsApp",
    instagram: "Instagram",
    language: "Langue",
    publishedMenu: "Menu numérique",
  },
  de: {
    navMenu: "Speisekarte",
    navInfo: "Informationen",
    reserve: "Tisch reservieren",
    menuEyebrow: "Unsere Speisekarte",
    searchLabel: "Speisekarte durchsuchen",
    searchPlaceholder: "Gericht oder Zutat suchen…",
    noResults: "Keine passenden Gerichte gefunden.",
    unavailable: "Nicht verfügbar",
    vegetarian: "Vegetarisch",
    vegan: "Vegan",
    glutenFree: "Glutenfrei",
    allergens: "Allergene",
    ingredients: "Zutaten",
    information: "Besuchen Sie uns",
    hours: "Öffnungszeiten",
    contacts: "Kontakt",
    directions: "Karte öffnen",
    whatsapp: "WhatsApp-Nachricht",
    instagram: "Instagram",
    language: "Sprache",
    publishedMenu: "Digitale Speisekarte",
  },
  es: {
    navMenu: "Menú",
    navInfo: "Información",
    reserve: "Reservar",
    menuEyebrow: "Nuestro menú",
    searchLabel: "Buscar en el menú",
    searchPlaceholder: "Busca un plato o ingrediente…",
    noResults: "No hay platos que coincidan con la búsqueda.",
    unavailable: "No disponible",
    vegetarian: "Vegetariano",
    vegan: "Vegano",
    glutenFree: "Sin gluten",
    allergens: "Alérgenos",
    ingredients: "Ingredientes",
    information: "Ven a visitarnos",
    hours: "Horarios",
    contacts: "Contacto",
    directions: "Abrir mapa",
    whatsapp: "Escríbenos por WhatsApp",
    instagram: "Instagram",
    language: "Idioma",
    publishedMenu: "Menú digital",
  },
};
